...
#############  fluent-bit-vali #############
FROM ghcr.io/fluent/fluent-operator/fluent-bit:4.0.1 AS fluent-bit-vali
...
#############      telegraf-builder       #############
FROM golang:1.24.4 AS telegraf-builder
...
#############      iptables-builder       #############
FROM alpine:3.22.0 AS iptables-builder
...
#############      tune2fs-builder       #############
FROM alpine:3.22.0 AS tune2fs-builder
