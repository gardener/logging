...
func NewPlugin(informer cache.SharedIndexInformer, cfg *config.Config, logger log.Logger) (Loki, error) {
	...
	loki.defaultClient, err = bufferedclient.NewClient(cfg, logger)
	if err != nil {
		return nil, err
	}
...
func (l *loki) SendRecord(r map[interface{}]interface{}, ts time.Time) error {
	...
	if !l.isDynamicHost(host) {
		...
		l.setDynamicTenant(records, lbs)
	}
...
func (l *loki) setDynamicTenant(record map[string]interface{}, lbs model.LabelSet) {
