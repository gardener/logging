...
https://github.com/credativ/vali/blob/v1.6.0/cmd/fluent-bit/out_vali.go
...
func FLBPluginRegister(ctx unsafe.Pointer) int {
	return output.FLBPluginRegister(ctx, "gardenervali", "Ship fluent-bit logs to Vali")
}
...
func FLBPluginInit(ctx unsafe.Pointer) int {
	...
	level.Info(paramLogger).Log("URL", conf.ClientConfig.ValiConfig.URL)
	level.Info(paramLogger).Log("TenantID", conf.ClientConfig.ValiConfig.TenantID)
	level.Info(paramLogger).Log("BatchWait", conf.ClientConfig.ValiConfig.BatchWait)
	level.Info(paramLogger).Log("BatchSize", conf.ClientConfig.ValiConfig.BatchSize)
	level.Info(paramLogger).Log("Labels", conf.ClientConfig.ValiConfig.ExternalLabels)
	...
	level.Info(paramLogger).Log("Timeout", fmt.Sprintf("%+v", conf.ClientConfig.ValiConfig.Timeout))
	level.Info(paramLogger).Log("MinBackoff", fmt.Sprintf("%+v", conf.ClientConfig.ValiConfig.BackoffConfig.MinBackoff))
	level.Info(paramLogger).Log("MaxBackoff", fmt.Sprintf("%+v", conf.ClientConfig.ValiConfig.BackoffConfig.MaxBackoff))
	level.Info(paramLogger).Log("MaxRetries", fmt.Sprintf("%+v", conf.ClientConfig.ValiConfig.BackoffConfig.MaxRetries))
